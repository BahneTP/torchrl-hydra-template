...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/bbf-testing/full_all_lr_sweep_with_shrink_perturb_jamesbond_seed1_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/bbf-testing/full_all_lr_sweep_with_shrink_perturb_jamesbond_seed1_${STAMP}}"
fi
...
game="Jamesbond"
seed=1
lrs=(1e-3 1e-4 1e-5 5e-6 1e-6)

"$PYTHON" scripts/lib/results.py init --results "$RESULTS_CSV" --summary "$SUMMARY_CSV"

for lr in "${lrs[@]}"; do
  lr_name="${lr//-/_}"
  VARIANT="resnet18_layer2_full_all_lr_${lr_name}_with_shrink_perturb"
  ...
  echo "Starting BBF ResNet18 layer2 full all_lr=${lr} with shrink-and-perturb ${game} seed ${seed} on GPU ${GPU}"
  set +e
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    echo "Run failed: ${game} seed ${seed} all_lr=${lr} (exit_code=${exit_code}). See ${log_file}"
    exit "$exit_code"
  fi
done
