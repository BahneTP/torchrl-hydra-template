...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/bbf-testing/full_without_shrink_perturb_jamesbond_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/bbf-testing/full_without_shrink_perturb_jamesbond_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="bbf"
VARIANT="resnet18_layer2_full_lr_1e_6_without_shrink_perturb"
...
for seed in "${seeds[@]}"; do
  ...
  echo "Starting BBF ResNet18 layer2 full fine-tuning without shrink-and-perturb ${game} seed ${seed} on GPU ${GPU}"
  set +e
