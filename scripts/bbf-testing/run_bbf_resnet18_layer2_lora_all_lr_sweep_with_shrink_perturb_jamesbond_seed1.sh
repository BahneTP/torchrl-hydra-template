...
RUN_ROOT="${RUN_ROOT:-logs/bbf-testing/lora_all_lr_sweep_with_shrink_perturb_jamesbond_seed1_${STAMP}}"
...
for lr in "${lrs[@]}"; do
  lr_name="${lr//-/_}"
  VARIANT="resnet18_layer2_lora_all_lr_${lr_name}_with_shrink_perturb"
  ...
  echo "Starting BBF ResNet18 layer2 LoRA all_lr=${lr} with shrink-and-perturb ${game} seed ${seed} on GPU ${GPU}"
  set +e
