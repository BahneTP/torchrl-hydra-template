...
RUN_ROOT="${RUN_ROOT:-logs/bbf-testing/full/full_all_lr_1e_5_without_shrink_perturb_jamesbond_seed1_${STAMP}}"
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="bbf"
VARIANT="resnet18_layer2_full_all_lr_1e_5_without_shrink_perturb"
game="Jamesbond"
seed=1
...
echo "Starting BBF ResNet18 layer2 full all_lr=1e-5 without shrink-and-perturb ${game} seed ${seed} on GPU ${GPU}"
set +e
CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
  ...
  algorithm.lr=1e-5 \
  algorithm.encoder_lr=1e-5 \
  algorithm.adapter_lr=1e-5 \
