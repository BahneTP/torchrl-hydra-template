...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/bbf-testing/attentive_probe_lr_sweep_with_shrink_perturb_jamesbond_seed1_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/bbf-testing/attentive_probe_lr_sweep_with_shrink_perturb_jamesbond_seed1_${STAMP}}"
fi
...
for probe_lr in "${lrs[@]}"; do
  lr_name="${probe_lr//-/_}"
  variant="resnet18_layer2_attentive_probe_lr_${lr_name}_with_shrink_perturb"
  ...
  echo "Starting BBF attentive probe_lr=${probe_lr} ${game} seed ${seed} on GPU ${GPU}"
  set +e
