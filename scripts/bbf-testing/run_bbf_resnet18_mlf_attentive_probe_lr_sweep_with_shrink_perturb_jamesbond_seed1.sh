...
RUN_ROOT="${RUN_ROOT:-logs/bbf-testing/mlf_attentive_probe_lr_sweep_with_shrink_perturb_jamesbond_seed1_${STAMP}}"
...
for probe_lr in "${lrs[@]}"; do
  lr_name="${probe_lr//-/_}"
  variant="resnet18_mlf_attentive_probe_lr_${lr_name}_with_shrink_perturb"
  ...
  echo "Starting BBF MLF attentive probe_lr=${probe_lr} ${game} seed ${seed} on GPU ${GPU}"
  set +e
