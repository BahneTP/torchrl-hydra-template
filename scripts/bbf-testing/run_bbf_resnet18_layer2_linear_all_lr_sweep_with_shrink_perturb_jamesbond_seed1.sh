...
RUN_ROOT="${RUN_ROOT:-logs/bbf-testing/linear_all_lr_sweep_with_shrink_perturb_jamesbond_seed1_${STAMP}}"
...
for lr in "${lrs[@]}"; do
  lr_name="${lr//-/_}"
  variant="resnet18_layer2_linear_all_lr_${lr_name}_with_shrink_perturb"
  ...
  echo "Starting BBF linear all_lr=${lr} ${game} seed ${seed} on GPU ${GPU}"
  set +e
