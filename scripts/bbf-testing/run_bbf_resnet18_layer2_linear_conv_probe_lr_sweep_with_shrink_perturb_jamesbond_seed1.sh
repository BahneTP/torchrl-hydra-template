...
RUN_ROOT="${RUN_ROOT:-logs/bbf-testing/linear_conv_probe_lr_sweep_with_shrink_perturb_jamesbond_seed1_${STAMP}}"
...
for probe_lr in "${lrs[@]}"; do
  lr_name="${probe_lr//-/_}"
  variant="resnet18_layer2_linear_conv_probe_lr_${lr_name}_with_shrink_perturb"
  ...
  echo "Starting BBF linear+conv probe_lr=${probe_lr} ${game} seed ${seed} on GPU ${GPU}"
  set +e
