...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/resnetlayer/der_resnet18_linear_layer2_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/resnetlayer/der_resnet18_linear_layer2_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="der"
VARIANT="resnet18_linear_layer2"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER ResNet18 linear layer 2 ${game} seed ${seed} on GPU ${GPU}"
    set +e
