...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/resnetlayer/der_resnet18_attentive_self_attention_layer2_probe_lr_1e_6_4games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/resnetlayer/der_resnet18_attentive_self_attention_layer2_probe_lr_1e_6_4games_${STAMP}}"
fi
...
PROBE_LR="1e-6"
VARIANT="resnet18_attentive_${PROBE_TYPE}_layer2_probe_lr_1e_6"
