...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/resnetlayer/attentive_lrsweep/der_resnet18_attentive_attention_weighted_pooling_layer2_probe_lr_sweep_jamesbond_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/resnetlayer/attentive_lrsweep/der_resnet18_attentive_attention_weighted_pooling_layer2_probe_lr_sweep_jamesbond_${STAMP}}"
fi
