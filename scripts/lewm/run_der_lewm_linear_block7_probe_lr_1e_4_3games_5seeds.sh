...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/lewm/experiment/der_lewm_linear_block7_probe_lr_1e_4_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/lewm/experiment/der_lewm_linear_block7_probe_lr_1e_4_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="der"
VARIANT="lewm_linear_block7_probe_lr_1e_4"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER LeWM linear block7 probe_lr=1e-4 adapter_lr=1e-4 ${game} seed ${seed} on GPU ${GPU}"
    set +e
