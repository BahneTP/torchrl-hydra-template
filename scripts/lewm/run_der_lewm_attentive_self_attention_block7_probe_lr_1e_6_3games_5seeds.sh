...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/lewm/experiment/der_lewm_attentive_self_attention_block7_probe_lr_1e_6_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/lewm/experiment/der_lewm_attentive_self_attention_block7_probe_lr_1e_6_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="der"
VARIANT="lewm_attentive_self_attention_block7_probe_lr_1e_6"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER LeWM attentive self_attention block7 probe_lr=1e-6 ${game} seed ${seed} on GPU ${GPU}"
    set +e
    CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
      experiment=der/lewm_attentive_atari100k \
      environment.task="$game" \
      algorithm.lewm_output_block=7 \
