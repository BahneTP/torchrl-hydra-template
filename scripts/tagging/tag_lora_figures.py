...
DEFAULT_RESULTS = [
    Path("logs/lora/der_resnet18_layer2_20260811_030006/results.csv"),
    Path("logs/lora/der_resnet18_layer4_20260809_001801/results.csv"),
    Path("logs/lora/der_dinov2_block12_20260809_095640/results.csv"),
]
...
def figure_tags_for(row: dict[str, str]) -> list[str]:
    ...
    if game == "Jamesbond":
        tags.append(f"fig-lora-jamesbond-v1-{slug(variant)}")

    if game == "Jamesbond" and variant.startswith("resnet18_lora_layer2_"):
        tags.append("fig-lora-jamesbond-v1-resnet18-layer2-rank-sweep")
    if game == "Jamesbond" and variant.startswith("resnet18_lora_layer4_"):
        tags.append("fig-lora-jamesbond-v1-resnet18-layer4-rank-sweep")
    if game == "Jamesbond" and variant.startswith("dinov2_lora_block12_"):
        tags.append("fig-lora-jamesbond-v1-dinov2-block12-rank-sweep")
...
def descriptive_tags_for(row: dict[str, str]) -> list[str]:
    variant = row["variant"]
    tags = ["tl", "der", "lora", slug(row["game"])]
    ...
    for token in variant.split("_"):
        if token.startswith("layer") or token.startswith("block"):
            tags.append(token)
    rank = re.search(r"_r(\d+)_a(\d+)$", variant)
    if rank:
        tags.extend([f"r{rank.group(1)}", f"a{rank.group(2)}"])
    return tags
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tag finished LoRA W&B runs for figure generation."
    )
    parser.add_argument(
        "--results",
        type=Path,
