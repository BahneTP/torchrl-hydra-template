...
DEFAULT_RESULTS = [
    Path("logs/mixedlayerfusion/der_resnet18_mlf_linear_20260808_155703/results.csv"),
    Path("logs/mixedlayerfusion/der_resnet18_mlf_attentive_20260808_155718/results.csv"),
    Path("logs/mixedlayerfusion/der_dinov2_mlf_linear_20260808_155743/results.csv"),
    Path("logs/mixedlayerfusion/der_dinov2_mlf_attentive_20260808_155758/results.csv"),
    Path("logs/mixedlayerfusion/der_resnet18_mlf_linear_all_3games_20260811_004533/results.csv"),
]
...
def figure_tags_for(row: dict[str, str], source: Path) -> list[str]:
    variant = row["variant"]
    game = row["game"]
    tags: list[str] = []

    if game == "Jamesbond" and "_mlf_" in variant:
        tags.append(f"fig-mlf-jamesbond-v1-{slug(variant)}")

    is_all_3games = "all_3games" in str(source)
    is_original_resnet18_linear = "der_resnet18_mlf_linear_20260808_155703" in str(source)
    if (
        variant == "resnet18_mlf_linear_all"
        and game in ATARI4_GAMES
        and ((game == "Jamesbond" and is_original_resnet18_linear) or (game != "Jamesbond" and is_all_3games))
    ):
        tags.append("fig-mlf-atari4-v1-resnet18-linear-all")
...
def descriptive_tags_for(row: dict[str, str]) -> list[str]:
    ...
    if variant.endswith("_all"):
        tags.append("all")
    if "layer1_4" in variant:
        tags.append("layer1-4")
    if "layer2_3" in variant:
        tags.append("layer2-3")
    if "first5" in variant:
        tags.append("first5")
    if "last5" in variant:
        tags.append("last5")
    if "block3_7_11" in variant:
        tags.append("block3-7-11")
    return tags
    ...
    for path in paths:
        ...
        with path.open(newline="") as f:
            for row in csv.DictReader(f):
                ...
                figure_tags = figure_tags_for(row, path)
                if not figure_tags:
                    continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tag finished mixed-layer-fusion W&B runs for figure generation."
    )
    parser.add_argument(
        "--results",
        type=Path,
