...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/fullfinetuning/der_dinov2_block12_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/fullfinetuning/der_dinov2_block12_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="der"
LRS="${LRS:-1e-4 1e-5 1e-6 1e-7 1e-8}"
...
for lr in $LRS; do
  lr_name="${lr//./p}"
  lr_name="${lr_name//-/_}"
  variant="dinov2_full_block12_lr_${lr_name}"
  for game in "${games[@]}"; do
    for seed in "${seeds[@]}"; do
      ...
      echo "Starting DER DINOv2 full block12 lr=${lr} ${game} seed ${seed} on GPU ${GPU}"
      set +e
      CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
        experiment=der/dinov2_full_atari100k \
