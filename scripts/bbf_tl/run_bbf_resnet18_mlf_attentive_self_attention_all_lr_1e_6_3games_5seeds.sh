...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/bbf_tl/bbf_resnet18_mlf_attentive_self_attention_all_lr_1e_6_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/bbf_tl/bbf_resnet18_mlf_attentive_self_attention_all_lr_1e_6_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="bbf"
VARIANT="resnet18_mlf_attentive_self_attention_all_lr_1e_6"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting BBF ResNet18 MLF attentive self-attention all layers ${game} seed ${seed} on GPU ${GPU}"
    set +e
    CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
      experiment=bbf/resnet18_mlf_attentive_atari100k \
      environment.task="$game" \
      evaluation.final_num_episodes=100 \
      algorithm.adapter_lr=1e-4 \
      algorithm.probe_lr=1e-6 \
      algorithm.resnet18_mix_layers=[1,2,3,4] \
