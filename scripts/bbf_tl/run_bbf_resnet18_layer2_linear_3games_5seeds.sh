...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/bbf_tl/bbf_resnet18_layer2_linear_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/bbf_tl/bbf_resnet18_layer2_linear_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="bbf"
VARIANT="resnet18_layer2_linear"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting BBF ResNet18 layer2 linear probing ${game} seed ${seed} on GPU ${GPU}"
    set +e
