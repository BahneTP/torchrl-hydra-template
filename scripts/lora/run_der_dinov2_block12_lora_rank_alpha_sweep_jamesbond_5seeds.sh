...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/lora/der_dinov2_block12_lora_rank_alpha_sweep_jamesbond_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/lora/der_dinov2_block12_lora_rank_alpha_sweep_jamesbond_${STAMP}}"
fi
...
for combo in "${combos[@]}"; do
  ...
  variant="dinov2_lora_block12_r${rank}_a${alpha}"
  for game in "${games[@]}"; do
    for seed in "${seeds[@]}"; do
      ...
      echo "Starting DER DINOv2 LoRA block12 rank=${rank} alpha=${alpha} ${game} seed ${seed} on GPU ${GPU}"
      set +e
