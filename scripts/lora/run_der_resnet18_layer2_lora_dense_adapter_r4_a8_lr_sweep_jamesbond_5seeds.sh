...
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/lora/lr_sweep \
    -maxdepth 1 -type d \
    -name 'der_resnet18_layer2_lora_dense_adapter_r4_a8_lr_sweep_jamesbond_*' \
    2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/lora/lr_sweep/der_resnet18_layer2_lora_dense_adapter_r4_a8_lr_sweep_jamesbond_${STAMP}}"
fi
...
for lr in "${lrs[@]}"; do
  lr_name="${lr//-/_}"
  variant="resnet18_lora_layer2_dense_adapter_r4_a8_lr_${lr_name}"
