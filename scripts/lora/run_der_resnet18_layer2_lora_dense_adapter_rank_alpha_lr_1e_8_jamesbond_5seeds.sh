...
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/lora/new_lora \
    -maxdepth 1 -type d \
    -name 'der_resnet18_layer2_lora_dense_adapter_rank_alpha_lr_1e_8_jamesbond_*' \
    2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/lora/new_lora/der_resnet18_layer2_lora_dense_adapter_rank_alpha_lr_1e_8_jamesbond_${STAMP}}"
fi
...
for combo in "${combos[@]}"; do
  ...
  variant="resnet18_lora_layer2_dense_adapter_r${rank}_a${alpha}_lr_1e_8"
  for seed in 1 2 3 4 5; do
    ...
    echo "Starting DER ResNet18 layer2 LoRA rank=${rank} alpha=${alpha} encoder_lr=1e-8 Jamesbond seed ${seed} on GPU ${GPU}"
    set +e
    CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
      experiment=der/resnet18_lora_atari100k environment.task=Jamesbond \
      algorithm.resnet18_variant=resnet_layer2 algorithm.lr=1e-4 \
      algorithm.encoder_lr=1e-8 algorithm.adapter_lr=1e-4 \
