...
RUN_ROOT="${RUN_ROOT:-logs/lora/new_lora/der_dinov2_block12_lora_rank_alpha_lr_1e_7_jamesbond_${STAMP}}"
...
for combo in "${combos[@]}"; do
  ...
  variant="dinov2_lora_block12_r${rank}_a${alpha}_lr_1e_7"
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER DINOv2 LoRA block12 rank=${rank} alpha=${alpha} lr=1e-7 Jamesbond seed ${seed} on GPU ${GPU}"
    set +e
