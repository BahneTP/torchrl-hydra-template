...
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/lora/new_lora \
    -maxdepth 1 -type d \
    -name 'der_dinov2_block12_lora_dense_adapter_r4_a8_lr_1e_8_1e_9_jamesbond_*' \
    2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/lora/new_lora/der_dinov2_block12_lora_dense_adapter_r4_a8_lr_1e_8_1e_9_jamesbond_${STAMP}}"
fi
...
for lr in "${lrs[@]}"; do
  ...
  for seed in 1 2 3 4 5; do
    ...
    echo "Starting DER DINOv2 block12 LoRA rank=4 alpha=8 encoder_lr=${lr} Jamesbond seed ${seed} on GPU ${GPU}"
    set +e
