...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/mixedlayerfusion/der_resnet18_mlf_linear_all_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/mixedlayerfusion/der_resnet18_mlf_linear_all_3games_${STAMP}}"
fi
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER ResNet18 MLF linear all layers ${game} seed ${seed} on GPU ${GPU}"
    set +e
