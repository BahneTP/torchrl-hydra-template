...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/mixedlayerfusion/der_dinov2_mlf_attentive_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/mixedlayerfusion/der_dinov2_mlf_attentive_${STAMP}}"
fi
...
for case_spec in "${cases[@]}"; do
  case_name="${case_spec%%|*}"
  blocks="${case_spec#*|}"
  variant="dinov2_mlf_${PROBE}_${case_name}"
  for game in "${games[@]}"; do
    for seed in "${seeds[@]}"; do
      ...
      echo "Starting DER DINOv2 MLF ${PROBE} ${case_name} ${game} seed ${seed} on GPU ${GPU}"
      set +e
