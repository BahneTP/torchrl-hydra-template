...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/mixedlayerfusion/der_resnet18_mlf_attentive_layer2_3_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/mixedlayerfusion/der_resnet18_mlf_attentive_layer2_3_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="der"
VARIANT="resnet18_mlf_attentive_layer2_3"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER ResNet18 MLF attentive layers 2+3 ${game} seed ${seed} on GPU ${GPU}"
    set +e
    CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
      experiment=der/resnet18_attentive_atari100k \
      environment.task="$game" \
      algorithm.transfer_layer_mix=true \
      'algorithm.resnet18_mix_layers=[2,3]' \
      trainer.seed="$seed" \
      trainer.devices=[0] \
      "run_name=$run_name" \
