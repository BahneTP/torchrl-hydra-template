...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest="$(find logs/bbf_tl_final/attentive_3games_5seeds_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest:-logs/bbf_tl_final/attentive_3games_5seeds_${STAMP}}"
fi
RESULTS="$RUN_ROOT/results.csv"
SUMMARY="$RUN_ROOT/summary.csv"
variant="resnet18_layer2_attentive_all_lr_1e_5"
...
for game in Assault BankHeist RoadRunner; do
  for seed in 1 2 3 4 5; do
    ...
    CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
      experiment=bbf/resnet18_layer2_attentive_atari100k environment.task="$game" \
      algorithm.lr=1e-5 algorithm.encoder_lr=1e-5 \
      algorithm.adapter_lr=1e-5 algorithm.probe_lr=1e-5 \
