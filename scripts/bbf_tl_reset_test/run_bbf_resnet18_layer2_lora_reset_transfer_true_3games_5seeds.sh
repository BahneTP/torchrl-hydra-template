...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/bbf_tl_reset_test/lora_reset_transfer_true_3games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/bbf_tl_reset_test/lora_reset_transfer_true_3games_${STAMP}}"
fi
RESULTS_CSV="$RUN_ROOT/results.csv"
SUMMARY_CSV="$RUN_ROOT/summary.csv"
ALGORITHM="bbf"
VARIANT="resnet18_layer2_lora_r4_a8_lr_1e_7_reset_transfer_true"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting BBF ResNet18 layer2 LoRA reset_transfer_encoder=true ${game} seed ${seed} on GPU ${GPU}"
    set +e
    CUDA_VISIBLE_DEVICES="$GPU" "$PYTHON" src/train.py \
      ...
      algorithm.reset_transfer_encoder=true \
      trainer.seed="$seed" \
      trainer.devices=[0] \
      "run_name=$run_name" \
