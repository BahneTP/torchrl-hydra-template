...
RUN_ROOT="${RUN_ROOT:-}"
if [[ -z "$RUN_ROOT" ]]; then
  latest_run_root="$(find logs/dinoblocks/der_dinov2_attentive_self_attention_block7_probe_lr_1e_6_4games_* -maxdepth 0 -type d 2>/dev/null | sort | tail -n 1 || true)"
  RUN_ROOT="${latest_run_root:-logs/dinoblocks/der_dinov2_attentive_self_attention_block7_probe_lr_1e_6_4games_${STAMP}}"
fi
...
PROBE_LR="1e-6"
VARIANT="dinov2_attentive_${PROBE_TYPE}_block7_probe_lr_1e_6"
...
for game in "${games[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    echo "Starting DER DINOv2 attentive ${PROBE_TYPE} block7 probe_lr=${PROBE_LR} ${game} seed ${seed} on GPU ${GPU}"
    set +e
