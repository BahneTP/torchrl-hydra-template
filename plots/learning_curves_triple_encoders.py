"""Render three encoder panels for one game with shared outer y-axes."""
...
def create_learning_curves_triple_encoders(
    ...
    *,
    ...
    """Render one game's curves for three encoders on a shared HNS scale."""
    if len(encoders) != 3:
        raise ValueError("exactly three encoders are required")
    ...
    if len(means) != 3 or len(stds) != 3:
        raise ValueError("means and stds must contain exactly three encoders")
    if mean_reward <= 0:
    ...
    fig, axes = plt.subplots(
        1, 3, figsize=(10.5, 3.6), sharey=True, constrained_layout=True
    )
    line_min = float("inf")
    line_max = float("-inf")
