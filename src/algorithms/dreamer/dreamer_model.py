...
class Dreamer(nn.Module):
    ...
    def update(
        ...
        if self._log_grads:
            ...
            grads = [
                p.grad for p in self._named_params.values() if p.grad is not None
    ...
    def _cal_grad(self, data, initial):
        ...
        if self.rep_loss == "dreamer":
            ...
            proto_losses = self.proto_loss(
                post_stoch_aug, post_deter_aug, embed_aug, ema_proj
            )
    ...
    def ema_update(self):
        ...
        if self._ema_updates % self.ema_update_every == 0:
            ...
            for s, d in zip(
                self.obs_proj.parameters(), self._ema_obs_proj.parameters()
            ):
                d.data.copy_(mix * s.data + (1 - mix) * d.data)
        self._ema_updates += 1
    ...
    def random_translate(self, x, max_delta, same_across_time=False, bilinear=False):
        ...
        arange_h = torch.linspace(
            -1.0 + eps_h, 1.0 - eps_h, h_padded, device=x.device, dtype=x.dtype
        )[:H]
        arange_w = torch.linspace(
            -1.0 + eps_w, 1.0 - eps_w, w_padded, device=x.device, dtype=x.dtype
        )[:W]
        ...
        if same_across_time:
            ...
            shift = torch.randint(
                0, 2 * pad + 1, size=(B * T, 1, 1, 2), device=x.device, dtype=x.dtype
            )

        shift = (
            shift
            * 2.0
            / torch.tensor([w_padded, h_padded], device=x.device, dtype=x.dtype)
        )

        # Apply shift and sample
        grid = base_grid + shift
        mode = "bilinear" if bilinear else "nearest"
        x_translated = F.grid_sample(
            x_padded, grid, mode=mode, padding_mode="zeros", align_corners=False
        )
